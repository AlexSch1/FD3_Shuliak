...
export const Them = React.createContext('ctx');
...
export function Shop2(props) {
    const [selectedItem, setSelectetItem] = useState(false);
    const [booksList, setBookList] = useState(props.books);
    const [stateViwe, setStateViwe] = useState(0);
    const [changeCard, setChangeCard] = useState(false);


    let onSelectItem = (book) => {
        // if (!this.alertSaveChange()) return;
        setSelectetItem(selectedItem===book.id ? false : book.id);
        setStateViwe(selectedItem === book.id ? 0 : 1);
    }
    ...
    let onDelete = (value) => {
        ...
        let newList = booksList.filter((v) => {
            return v.id !== value;
        });

        setBookList(newList);
        setStateViwe(selectedItem === value ? 0 : 1);
    ...
    let selectedItemID = selectedItem;
    ...
    return (
        ...
        <div className="inner">
            <h1 className="inner__title">{props.titleTable}</h1>
            <div className="inner__list">{list}</div>

            {(stateViwe <= 1) && <button style={{ margin: '20px' }} >NEW BOOK</button>}

            {(stateViwe === 1) && <CardInfo book={selectedItemProps} />}
    ...
    render() {
        ...
        if (this.state.stateViwe === 3) { // если создаем новый товар - нужен шаблон объекта товара с пустыми полями
            for (let i in booksList[0]) {
                if (i === 'id') {
                    selectedItemProps[i] = generateNewIdBook(booksList);
                } else {
                    selectedItemProps[i] = '';
                }
            }
